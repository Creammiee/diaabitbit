...
current_label = tk.Label(root, text="Glucose: -- mg/dL", font=("Helvetica", 18))
current_label.pack(pady=5)

fig = Figure(figsize=(4.2, 1.8), dpi=80)
ax = fig.add_subplot(111)
ax.set_title("Forecasted Glucose", fontsize=10)
ax.set_xlabel("Time (min)", fontsize=8)
ax.set_ylabel("Glucose (mg/dL)", fontsize=8)
ax.grid(True)
canvas = FigureCanvasTkAgg(fig, master=root)
canvas.get_tk_widget().pack(pady=5, fill=tk.BOTH, expand=True)
...
def get_hr_ibi(duration=60):
    ...
    while time.time() - start_time < duration:
        ...
        if len(ir_buffer) > 250:
            ir_buffer.pop(0); time_buffer.pop(0)
        time.sleep(0.005)
    signal = ir_buffer
    t = time_buffer
...
def update_forecast():
    ...
    current_label.config(text=f"Glucose: {predicted_glucose:.2f} mg/dL")
    ax.clear()
    ax.plot(horizons, forecasted_glucose, marker='o', linestyle='-', color='blue')
    ax.set_title("Forecasted Glucose", fontsize=10)
    ax.set_xlabel("Time (min)", fontsize=8)
    ax.set_ylabel("Glucose (mg/dL)", fontsize=8)
...
def reset(channel=None):
    global last_predicted_glucose
    last_predicted_glucose = 123.0
    ax.clear()
    ax.set_title("Forecasted Glucose", fontsize=10)
    ax.set_xlabel("Time (min)", fontsize=8)
    ...
    ax.grid(True)
    canvas.draw()
    current_label.config(text="Glucose: -- mg/dL")
...
GPIO.add_event_detect(RESET_BTN, GPIO.FALLING, callback=reset, bouncetime=300)
